...
router.post('/register' , async(req,res)=>{
    try{
        let {email,username,role,qualification,dob,address,state,city,pin,contact,password} = req.body;
        const user = new User({email,username,role,qualification,dob,address,state,city,pin,contact});
        const newUser = await User.register(user , password );
        // res.redirect('/login');
        req.login( newUser , function(err){
            if(err){return next(err)}
            req.flash('success' , 'welcome,  you are registed succesfully');
            return res.redirect('/edtech');
        })
    }
    catch(e){
